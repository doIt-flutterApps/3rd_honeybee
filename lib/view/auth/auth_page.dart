...
class _AuthPage extends State<AuthPage> {
  ...
  void _signUp() async {
    try {
      // createUserWithEmailAndPassword 메서드로 회원 가입 요청하기
      await _auth.createUserWithEmailAndPassword(
        email: _emailController.text,
        password: _passwordController.text,
      );
      // 회원 가입이 성공이면 메시지 업데이트하기
      setState(() {
  ...
  void _signIn() async {
    try {
      // signInWithEmailAndPassword 메서드로 로그인 요청하기
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              SizedBox(height: 20),
              ElevatedButton(onPressed: _findPassword, child: Text('비밀번호 찾기')),
            ],
          ),
        ),
      ),
